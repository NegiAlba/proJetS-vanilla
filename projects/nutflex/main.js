...
class Serie {
    ...
    serieDuration(){
        ...
        return `${hours}h${minutes} au total`;
    }

    summary(){
        const summary = this.synopsis.substr(0,200);
        return summary;
    }
  ...
  window.addEventListener("DOMContentLoaded",()=>{
    for(let serie in data){
      ...
      card.className = "card";
      card.style.background = `no-repeat center/80% #555 url(${aSerie.img})`;
      console.log(card);
        ...
        const cardSeasonsEpisodes = document.createElement('p');
        const cardBinge = document.createElement('p');
        const cardFavorite = document.createElement('button');
        const cardTrailer = document.createElement('button');

        cardTitle.id = "card-title";
        cardSynopsis.id = "card-synopsis";
        cardSeasonsEpisodes.id = "card-seasons-episodes";
        cardBinge.id = "card-binge";
        cardFavorite.id = "card-favorite";
        cardTrailer.id = "card-trailer";

        cardTitle.innerText = `${aSerie.title}`;
        cardSynopsis.innerText = `${aSerie.summary()}...`;
        cardSeasonsEpisodes.innerText = `${aSerie.seasons} de ${aSerie.episodes}`;
        cardBinge.innerText = `${aSerie.serieDuration()}`;
        cardFavorite.innerHTML = '<i class="fas fa-heart"></i>';
        cardTrailer.innerHTML = '<i class="fas fa-video"></i>';

        cardOverlay.appendChild(cardHeader);
        cardHeader.appendChild(cardTitle);
        cardOverlay.appendChild(cardContent);
        cardContent.appendChild(cardSynopsis);
        cardOverlay.appendChild(cardButtons);
        cardButtons.appendChild(cardFavorite);
        cardButtons.appendChild(cardTrailer);
        cardOverlay.appendChild(cardFooter);
        cardFooter.appendChild(cardSeasonsEpisodes);
        cardFooter.appendChild(cardBinge);
        console.log(card.style.background = `${aSerie.img}`)
...
const season = document.querySelector("#card-seasons-episodes");
// const episodeNumber = document.querySelector("#episode")
// const durationEpisode = document.querySelector("#duration");
const synopsis = document.querySelector("#card-synopsis");
const totalDuration = document.querySelector("#card-binge");